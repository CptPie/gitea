...
// FullDeleteUser deletes models associated to an user. It also anonymizes mentions.
func FullDeleteUser(ctx context.Context, u *user_model.User) (err error) {
	e := db.GetEngine(ctx)
	...
	// ***** START: Mentions *****
	{
		...
		for start := 0; ; start += batchSize {
			comments := make([]*Comment, 0, batchSize)
			if err = e.Limit(batchSize, start).Find(&comments); err != nil {
				return err
			}
			if len(comments) == 0 {
				break
			...
			for _, comment := range comments {
				if strings.Contains(comment.Content, u.Name) {
					err = removeMentionFromComment(ctx, comment, u)
					if err != nil {
						return err
					}
				}
			...
			if err = e.Limit(batchSize, start).Find(&issues); err != nil {
				return err
			}
			if len(issues) == 0 {
				break
			...
			for _, issue := range issues {
				if strings.Contains(issue.Content, u.Name) {
					err = removeMentionFromIssue(ctx, issue, u)
					if err != nil {
						return err
					}
				}
...
func removeMentionFromIssue(ctx context.Context, issue *Issue, user *user_model.User) error {
	issue.Content = strings.ReplaceAll(issue.Content, "@"+user.Name, "@ghost")
...
func removeMentionFromComment(ctx context.Context, comment *Comment, user *user_model.User) error {
	comment.Content = strings.ReplaceAll(comment.Content, "@"+user.Name, "@ghost")
	sess := db.GetEngine(ctx)
	if _, err := sess.ID(comment.ID).AllCols().Update(comment); err != nil {
		return err
	}
	return nil
}
